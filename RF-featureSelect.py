...
def main():
    ...
    CROSSVALIDATION = 10
    CPUCHANNEL = 6
    print("estimate:Random Forest \t step:{0} \t min_feature_to_select:{1} \t CrossValidation:{2} \t CPUChannel:{3} \n"
          .format(STEPSET,MINFEATURETOSET,CROSSVALIDATION,CPUCHANNEL))
    
    # 特征选择
    print('*'*20,"读取特征文件",'*'*20)
    maskSaveName = "RF-features-mask.out"
...
#    plt.show()
    ...
    clf_selected = RandomForestClassifier(n_estimators = N_EStimators, max_depth = MAX_DEPTH)
